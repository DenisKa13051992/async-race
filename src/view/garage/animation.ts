...
const animation = (id: number) => {
  ...
  valuesForMoving(idCarAnimation).then(() => {
    animationId[idCarAnimation] = animationMove(carHTML, way, time);
  });
  driveCarAPI(idCarAnimation).then(status => {
    if (!status.success) {
      window.cancelAnimationFrame(animationId[idCarAnimation].id);
    }
  });
};
...
const animationMove = (carHTML: HTMLElement, way: number, time: number) => {
  ...
  const getStep = (times: number) => {
    ...
    if (progressWay < way) {
      ...
      carHTML.style.transform = `translateX(${way}px)`;
    }
    if (
      progressWay >= way &&
...
btnRace.addEventListener('click', () => {
  ...
  for (const carStart of carAllStart) {
    carStart.disabled = true;
  }
  for (const carStop of carAllStop) {
    carStop.disabled = false;
  }
});
...
btnReset.addEventListener('click', () => {
  ...
  stopRace(pageNumber);
  for (const carsImg of AllCarsImg) {
...
export const changeWinMessage = (change: boolean) => {
  if (!change) {
    winnerMessageShow = false;
  } else winnerMessageShow = true;
};
