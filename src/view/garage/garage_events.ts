...
import { changeWinMessage, winnerMessage } from './animation';
import {
  createCarAPI,
  cars,
  getCarsAPI,
...
export const checkCars = () => {
  getCarsAPI(pageNumber).then(() => {
    if (cars / 7 > pageNumber) {
...
generateCars.addEventListener('click', event => {
  ...
  if (target.closest('.btn-update')) {
    updateCarAPI(
      { name: inputCreateNewName.value, color: inputCreateNewColor.value },
      updatedCarId,
    ).then(() => updateCarsUI());
...
btnNext.addEventListener('click', () => {
  if (cars / 7 > pageNumber) {
    pageNumber += 1;
    updateCarsUI();
    pageNumberHTML.textContent = String(pageNumber);
    if (cars / 7 < pageNumber) {
      btnNext.disabled = true;
      console.log(cars / 7, pageNumber);
    }
  }
  checkCars();
});
...
carsContainer.addEventListener('click', event => {
  ...
  if (target.closest('.car-select')) {
    ...
    const selectCarName = <HTMLElement>document.getElementById(`car_name-${updatedCarId}`);
