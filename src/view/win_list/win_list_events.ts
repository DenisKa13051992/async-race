...
export const checkCarsWinners = () => {
  ...
  getWinnersAPI(pageNumberWinners).then(() => {
    if (carsWinners / 10 > pageNumberWinners) {
      btnNextWin.disabled = false;
    }
    if (pageNumberWinners === 1) {
      btnPrevWin.disabled = true;
    }
    if (pageNumberWinners > 1) {
...
btnWinners.addEventListener('click', () => {
  ...
  getWinnersAPI(pageNumberWinners).then((arr: ArraysOfCars[]) => {
    for (const car of arr) {
      let winName: string;
      let winColor: string;
      const winWins = car.wins;
      const winTime = car.time;

      getCarAPIByIdFromGarage(car.id)
        .then((ar: ArraysOfCars) => {
          winName = ar.name;
          winColor = ar.color;
        })
        .then(() => {
          num += 1;
          winCarsForRender += createWinnerUI(
            num,
            winColor,
            winName,
            winWins,
...
btnNextWin.addEventListener('click', () => {
  if (carsWinners / 10 > pageNumberWinners) {
    pageNumberWinners += 1;
    swichWinPage();
    pageNumberWinnersHTML.textContent = String(pageNumberWinners);
    btnPrevWin.disabled = false;
    if (carsWinners / 10 < pageNumberWinners) {
      btnNextWin.disabled = true;
    }
